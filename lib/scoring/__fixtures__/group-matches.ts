// Casos canónicos de §3.1 (marcadores de fase de grupos), regla v2.2 del Excel
// (ADR 0009 + ADR 0012). Cada caso es una tripleta predicción + oficial →
// esperado, con el esperado calculado A MANO desde la doc. NO derivar el
// esperado con la función que se testea: este fixture es la red de seguridad.
//
// Tabla: 8 (exacto = 3 signo + 5 exacto) / 3 (signo 1X2 sin marcador exacto) /
// 0 (resto). Sin penalización por hueco (v2.0 eliminó la categoría `penalties`).
...
export const groupMatchCases: GroupMatchCase[] = [
  ...
  {
    name: 'fallo de 1X2 acertando los goles del local (2-1 vs 2-3) → 0 (v2.2 no premia diferencia)',
    prediction: { golesLocal: 2, golesVisitante: 1 },
